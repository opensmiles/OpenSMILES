...
Geoff Hutchison, Craig A. James, Greg Landrum, Chris Morley, Egon Willighagen, Hans De Winter, Tim Vandermeersch, John Mayfield
...
both bonded atoms, the two bond types must be the same.
...
Directional up/down bonds that are used to specify double bond configuration are 
relative to the atoms they connect. On ring closures this means for bonds to be
the same type the the symbol inverts (`'/' => '\'` and `'\' => '/'`). If the 
symbols are the same the interpretation is implementation dependant. Some 
parsers will choose to ignore either the ring open or ring close, ideally such 
structures should be returned with undefined configuration and a warning issued. 
Alternatively the SMILES may be rejected.

For this reason it is preferred to only specify the bond type on either the open
or the close. Unfortunately this can lead to issues when generating SMILES where
a double bond is accidentality set as unspecified due to the specification of
adjacent double bonds. 

[options="header",frame="topbot",grid="rows",width="90%"]
|=================================================================================================
| SMILES                  | Name
| `C/1CCCCCCCCC/C=C\1`    | Z-cyclododecanene
| `C/1CCCCCCCCC/C=C1`     | Z-cyclododecanene
| `C1CCCCCCCCC/C=C\1`     | Z-cyclododecanene
| `C\1CCCCCCCCC/C=C/1`    | E-cyclododecanene
| `C\1CCCCCCCCC/C=C1`     | E-cyclododecanene
| `C1CCCCCCCCC/C=C/1`     | E-cyclododecanene
| `C\1CCCCCCCCC/C=C\1`    | cyclododecanene *undefined configuration*
| `C/1CCCCCCCCC/C=C/1`    | cyclododecanene *undefined configuration*
|=================================================================================================
